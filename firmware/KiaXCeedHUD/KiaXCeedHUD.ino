...
static void stopWeb(){dns.stop();WiFi.softAPdisconnect(true);accessGate.revoke();apPassword="";apSsid="";lv_obj_add_flag(qrCanvas,LV_OBJ_FLAG_HIDDEN);lv_obj_add_flag(accessLabel,LV_OBJ_FLAG_HIDDEN);WiFi.mode(WIFI_STA);if(!cfg.ssid.length())WiFi.disconnect(false,false);}
...
static void initWifi(){WiFi.mode(WIFI_STA);WiFi.setHostname(cfg.hostname.c_str());if(cfg.ssid.length())WiFi.begin(cfg.ssid,cfg.password);else WiFi.disconnect(false,false);}
static void routes(){
