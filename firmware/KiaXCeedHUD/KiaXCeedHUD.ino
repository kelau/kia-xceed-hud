...
static void triggerBeeper(uint32_t durationMs){boardServices.beep(durationMs);}
...
static void armWeb(lv_event_t*){stopWeb();accessGate.arm(millis());apSsid="Kia-HUD-"+randomSecret(4);apPassword=randomSecret(20);WiFi.mode(cfg.ssid.length()?WIFI_AP_STA:WIFI_AP);WiFi.softAP(apSsid.c_str(),apPassword.c_str(),1,0,1);dns.start(53,"*",WiFi.softAPIP());showQr("WIFI:T:WPA;S:"+apSsid+";P:"+apPassword+";;");qrShownAt=millis();lv_label_set_text_fmt(accessLabel,"Scan to join %s · tap screen to close",apSsid.c_str());lv_obj_clear_flag(qrBackdrop,LV_OBJ_FLAG_HIDDEN);lv_obj_clear_flag(accessLabel,LV_OBJ_FLAG_HIDDEN);lv_obj_move_foreground(qrBackdrop);lv_obj_move_foreground(qrCanvas);lv_obj_move_foreground(accessLabel);}
...
static void routes(){
  ...
  server.on("/api/compatibility",HTTP_GET,[](AsyncWebServerRequest*r){if(!auth(r))return;JsonDocument j;j["firmwareVersion"]=FIRMWARE_VERSION;j["uiVersion"]=FIRMWARE_VERSION;j["apiVersion"]=2;j["configSchemaVersion"]=CONFIG_SCHEMA_VERSION;j["activeDisplay"]=activeVirtualDisplay<0?"main":cfg.virtualDisplays[activeVirtualDisplay].id;auto c=j["capabilities"].to<JsonArray>();c.add("event-stream");c.add("virtual-displays");c.add("asset-storage");c.add("ui-diagnostics");sendJson(r,j);});
  ...
  server.on("/api/resources",HTTP_GET,[](AsyncWebServerRequest*r){if(!auth(r))return;JsonDocument j;auto a=j.to<JsonArray>();for(const char*name:{"builtin:warning","builtin:battery","builtin:gps","builtin:car","builtin:temperature","builtin:wifi","builtin:check","builtin:info"})a.add(name);if(boardServices.sdMounted()){File dir=SD_MMC.open("/hud/icons");for(File f=dir.openNextFile();f;f=dir.openNextFile())if(!f.isDirectory()){String path=f.path();if(path.endsWith(".rgb565"))a.add(path);}}sendJson(r,j);});
  server.on("/api/resources/upload",HTTP_POST,[](AsyncWebServerRequest*r){if(!auth(r))return;if(!boardServices.sdMounted()){r->send(503,"application/json","{\"error\":\"TF card unavailable\"}");return;}String*path=(String*)r->_tempObject;bool uploaded=path!=nullptr;JsonDocument j;j["uploaded"]=uploaded;j["path"]=uploaded?*path:"";String response;serializeJson(j,response);delete path;r->_tempObject=nullptr;r->send(uploaded?201:500,"application/json",response);},[](AsyncWebServerRequest*r,String filename,size_t index,uint8_t*data,size_t len,bool final){if(!boardServices.sdMounted())return;filename.replace("\\","_");filename.replace("/","_");filename.replace("..","_");if(!filename.endsWith(".rgb565"))filename+=".rgb565";String path="/hud/icons/"+filename;if(!index){r->_tempObject=new String(path);r->_tempFile=SD_MMC.open(path,FILE_WRITE);}if(r->_tempFile)r->_tempFile.write(data,len);if(final&&r->_tempFile)r->_tempFile.close();});
  server.on("/api/status",HTTP_GET,[](AsyncWebServerRequest*r){if(!auth(r))return;JsonDocument j;j["version"]=FIRMWARE_VERSION;time_t now=time(nullptr);char date[40]="Not synchronized";if(now>1700000000){struct tm local;localtime_r(&now,&local);strftime(date,sizeof(date),"%Y-%m-%d %H:%M:%S %Z",&local);}j["date"]=date;j["uptime"]=millis()/1000;j["heap"]=ESP.getFreeHeap();j["minHeap"]=ESP.getMinFreeHeap();j["psramFree"]=ESP.getFreePsram();j["flashSize"]=ESP.getFlashChipSize();j["cpuMHz"]=ESP.getCpuFreqMHz();j["sdk"]=ESP.getSdkVersion();j["sdMounted"]=boardServices.sdMounted();j["sdWritable"]=boardServices.sdWritable();j["sdStage"]=boardServices.sdStage();j["sdAttempts"]=boardServices.sdAttempts();j["sdFrequencyKhz"]=boardServices.sdFrequencyKhz();j["client"]=r->client()->remoteIP().toString();j["ip"]=WiFi.localIP().toString();j["rssi"]=WiFi.RSSI();j["canAge"]=(micros()-telemetry.lastCanUs)/1000.0f;j["frameCount"]=frameCount;j["displayConfigRequested"]=displayConfigRequested;j["displayConfigApplied"]=displayConfigApplied;j["simulateObd"]=cfg.simulateObd;j["listenOnly"]=cfg.canListenOnly;j["speed"]=telemetry.speedKph;j["rpm"]=telemetry.rpm;j["soc"]=telemetry.soc;j["coolant"]=telemetry.coolantC;j["load"]=telemetry.engineLoad;j["trip"]=telemetry.tripKm;j["gps"]=telemetry.gpsFix;sendJson(r,j);});
  ...
  server.on("/api/config",HTTP_PUT,[](AsyncWebServerRequest*r){},nullptr,[](AsyncWebServerRequest*r,uint8_t*d,size_t n,size_t index,size_t total){if(index==0){if(!auth(r))return;r->_tempObject=new String();((String*)r->_tempObject)->reserve(total);}auto body=(String*)r->_tempObject;if(!body)return;body->concat((const char*)d,n);if(index+n<total)return;Config*candidate=(Config*)heap_caps_malloc(sizeof(Config),MALLOC_CAP_SPIRAM);if(!candidate){delete body;r->_tempObject=nullptr;r->send(503,"application/json","{\"error\":\"configuration memory unavailable\"}");return;}xSemaphoreTake(configMutex,portMAX_DELAY);new(candidate)Config(cfg);xSemaphoreGive(configMutex);bool parsed=candidate->fromJson(*body),stored=parsed&&candidate->save();uint32_t revision=0;if(stored){xSemaphoreTake(configMutex,portMAX_DELAY);cfg=*candidate;revision=++displayConfigRequested;xSemaphoreGive(configMutex);}candidate->~Config();free(candidate);delete body;r->_tempObject=nullptr;if(!parsed){r->send(400,"application/json","{\"error\":\"config parse failed\"}");return;}if(!stored){r->send(500,"application/json","{\"error\":\"config storage failed\"}");return;}char response[64];snprintf(response,sizeof(response),"{\"saved\":true,\"displayRevision\":%u}",(unsigned)revision);r->send(200,"application/json",response);events.send(response,"config-saved",millis());});
...
void loop() {
  ...
  while(bus.receive(f)) {
    ...
    frameCount=min(frameCount+1,frames.size());frameTotal++;telemetry.lastCanMs=f.ms;telemetry.lastCanUs=micros();
    decodeObd(f,telemetry);
  }
  static uint32_t nextSimUs=0,simSequence=0;
  if(cfg.simulateObd){uint32_t nowUs=micros();if(!nextSimUs)nextSimUs=nowUs;uint8_t emitted=0;while((int32_t)(nowUs-nextSimUs)>=0&&emitted<SIMULATED_CAN_MAX_CATCH_UP){f=simulatedFrame(millis(),simSequence++);if(frameCount==frames.size())performance->noteCanCoalesced();frames[frameHead]=f;frameHead=(frameHead+1)%frames.size();frameCount=min(frameCount+1,frames.size());frameTotal++;telemetry.lastCanMs=f.ms;telemetry.lastCanUs=micros();decodeObd(f,telemetry);nextSimUs+=SIMULATED_CAN_PERIOD_US;emitted++;}if((int32_t)(nowUs-nextSimUs)>100000){performance->noteCanDropped((nowUs-nextSimUs)/SIMULATED_CAN_PERIOD_US);nextSimUs=nowUs;}}else nextSimUs=0;
  ...
  if(eventNow-lastFramesEvent>=250){lastFramesEvent=eventNow;events.send("{}","frames",eventNow);}
