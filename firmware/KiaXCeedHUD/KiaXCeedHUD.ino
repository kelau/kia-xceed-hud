...
static void routes(){
  ...
  server.on("/api/display-config",HTTP_GET,[](AsyncWebServerRequest*r){if(!auth(r))return;String s;displayCfg.toJson(s);r->send(200,"application/json",s);});
  server.on("/api/config",HTTP_PUT,[](AsyncWebServerRequest*r){},nullptr,[](AsyncWebServerRequest*r,uint8_t*d,size_t n,size_t index,size_t total){if(index==0){if(!auth(r))return;r->_tempObject=new String();((String*)r->_tempObject)->reserve(total);}auto body=(String*)r->_tempObject;if(!body)return;body->concat((const char*)d,n);if(index+n<total)return;xSemaphoreTake(configMutex,portMAX_DELAY);Config candidate=cfg;xSemaphoreGive(configMutex);bool parsed=candidate.fromJson(*body),stored=parsed&&candidate.save();uint32_t revision=0;if(stored){xSemaphoreTake(configMutex,portMAX_DELAY);cfg=candidate;revision=++displayConfigRequested;xSemaphoreGive(configMutex);}delete body;r->_tempObject=nullptr;if(!parsed){r->send(400,"application/json","{\"error\":\"config parse failed\"}");return;}if(!stored){r->send(500,"application/json","{\"error\":\"config storage failed\"}");return;}char response[64];snprintf(response,sizeof(response),"{\"saved\":true,\"displayRevision\":%u}",(unsigned)revision);r->send(200,"application/json",response);events.send(response,"config-saved",millis());});
...
void setup(){
  ...
  cfg.load();displayCfg=cfg;bootDisplayUntil=millis()+cfg.virtualDisplays[0].timeoutMs;if(cfg.virtualDisplays[0].enabled){activeVirtualDisplay=0;displayCfg.widgets=cfg.virtualDisplays[0].widgets;}Serial.printf("Config schema %u loaded\n",CONFIG_SCHEMA_VERSION);
...
void loop() {
  ...
  if(displayConfigApplied!=displayConfigRequested||desiredDisplay!=activeVirtualDisplay){xSemaphoreTake(configMutex,portMAX_DELAY);displayCfg=cfg;uint32_t revision=displayConfigRequested;if(desiredDisplay>=0)displayCfg.widgets=cfg.virtualDisplays[desiredDisplay].widgets;xSemaphoreGive(configMutex);dashboard.rebuild(displayCfg);dashboard.update(displayCfg,telemetry,millis());dashboard.sendToBack();display.setBrightness(displayCfg.brightness);activeVirtualDisplay=desiredDisplay;displayConfigApplied=revision;Serial.printf("Display revision %u applied; virtual display %d\n",(unsigned)revision,(int)activeVirtualDisplay);}
  updateTestNetwork();
