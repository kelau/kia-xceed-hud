...
struct Config {
  ...
  uint8_t brightness=80; std::array<Widget,40> widgets{{
  ...
  void toJson(String& out,bool includeSecrets=false)const{JsonDocument d;d["hostname"]=hostname;d["ssid"]=ssid;d["otaSsid"]=otaSsid;d["otaPasswordSet"]=otaPassword.length()>0;if(includeSecrets){d["password"]=password;d["otaPassword"]=otaPassword;}d["canListenOnly"]=canListenOnly;d["simulateObd"]=simulateObd;d["gpsEnabled"]=gpsEnabled;d["imuEnabled"]=imuEnabled;d["brightness"]=brightness;auto a=d["widgets"].to<JsonArray>();for(auto&w:widgets){if(w.id.empty())continue;auto o=a.add<JsonObject>();o["id"]=w.id;o["x"]=w.x;o["y"]=w.y;o["w"]=w.w;o["h"]=w.h;o["visible"]=w.visible;o["title"]=w.title;o["fontSize"]=w.fontSize;o["background"]=w.background;o["textColor"]=w.textColor;o["backgroundOpacity"]=w.backgroundOpacity;o["border"]=w.border;o["decimals"]=w.decimals==255?widgetDefaultDecimals(w.id):w.decimals;o["valueAlign"]=w.valueAlign;o["timeFormat"]=w.timeFormat;o["visualStyle"]=w.visualStyle;o["visualMin"]=w.visualMin;o["visualMax"]=w.visualMax;o["lightLow"]=w.lightLow;o["lightHigh"]=w.lightHigh;o["trackColor"]=w.trackColor;o["accentColor"]=w.accentColor;o["lowColor"]=w.lowColor;o["midColor"]=w.midColor;o["highColor"]=w.highColor;o["visualThickness"]=w.visualThickness;o["subdivisions"]=w.subdivisions;}serializeJson(d,out);}
  bool fromJson(const String&s){JsonDocument d;if(deserializeJson(d,s))return false;widgets={};hostname=d["hostname"]|hostname;ssid=d["ssid"]|ssid;const char*p=d["password"]|"";if(*p)password=p;otaSsid=d["otaSsid"]|otaSsid;const char*op=d["otaPassword"]|"";if(*op)otaPassword=op;canListenOnly=d["canListenOnly"]|true;simulateObd=d["simulateObd"]|false;gpsEnabled=d["gpsEnabled"]|true;imuEnabled=d["imuEnabled"]|true;brightness=constrain((int)(d["brightness"]|80),5,100);int i=0;for(JsonObject o:d["widgets"].as<JsonArray>()){if(i>=40)break;Widget w;w.id=std::string((const char*)(o["id"]|""));if(w.id.empty())return false;w.w=constrain((int)(o["w"]|100),40,480);w.h=constrain((int)(o["h"]|70),30,480);w.x=constrain((int)(o["x"]|0),0,480-w.w);w.y=constrain((int)(o["y"]|0),0,480-w.h);w.visible=o["visible"]|true;strlcpy(w.title,o["title"]|"",sizeof(w.title));w.fontSize=(uint8_t)constrain((int)(o["fontSize"]|18),10,48);w.background=(uint32_t)(o["background"]|0x102a38);w.textColor=(uint32_t)(o["textColor"]|0xffffff);w.backgroundOpacity=(uint8_t)constrain((int)(o["backgroundOpacity"]|255),0,255);w.border=o["border"]|true;w.decimals=(uint8_t)constrain((int)(o["decimals"]|widgetDefaultDecimals(w.id)),0,5);w.valueAlign=(uint8_t)constrain((int)(o["valueAlign"]|1),0,2);w.timeFormat=(uint8_t)constrain((int)(o["timeFormat"]|0),0,3);w.visualStyle=(uint8_t)constrain((int)(o["visualStyle"]|WIDGET_VALUE),0,3);w.visualMin=o["visualMin"]|0.0f;w.visualMax=o["visualMax"]|0.0f;w.lightLow=o["lightLow"]|0.0f;w.lightHigh=o["lightHigh"]|0.0f;w.trackColor=(uint32_t)(o["trackColor"]|0x1f3b4d);w.accentColor=(uint32_t)(o["accentColor"]|0x2dd4bf);w.lowColor=(uint32_t)(o["lowColor"]|0xef4444);w.midColor=(uint32_t)(o["midColor"]|0xf59e0b);w.highColor=(uint32_t)(o["highColor"]|0x22c55e);w.visualThickness=(uint8_t)constrain((int)(o["visualThickness"]|8),2,128);w.subdivisions=(uint8_t)constrain((int)(o["subdivisions"]|0),0,20);widgets[i++]=w;}return i>0;}
};}
