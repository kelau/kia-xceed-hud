...
struct Config {
  ...
  static bool widgetsFromJson(JsonArrayConst a,std::array<Widget,40>&widgets){widgets={};int i=0;for(JsonObjectConst item:a){if(i>=40)break;Widget w;if(!widgetFromJson(item,w))return false;widgets[i++]=w;}return i>0;}
