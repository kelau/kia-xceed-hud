...
class Dashboard {
 public:
  ...
  void rebuild(const Config& c){
    auto old=root_;auto next=createRoot();cards_.fill(nullptr);titles_.fill(nullptr);values_.fill(nullptr);
    populate(next,c);root_=next;lv_obj_move_background(root_);if(old)lv_obj_del(old);
  }
  void populate(lv_obj_t* root,const Config& c){
    for(size_t i=0;i<c.widgets.size();i++){
      auto&w=c.widgets[i];if(!w.visible)continue;
      auto card=lv_obj_create(root);cards_[i]=card;titles_[i]=lv_label_create(card);values_[i]=lv_label_create(card);
      ...
      auto value=values_[i];lv_obj_set_style_text_color(value,lv_color_hex(w.textColor),0);lv_obj_align(value,LV_ALIGN_CENTER,0,8);
    }
  }
  void update(const Config& c,const Telemetry&t,uint32_t now){
    for(size_t i=0;i<c.widgets.size();i++){
      ...
      else if(id=="time"||id=="date"){time_t epoch=time(nullptr);if(epoch>1700000000){struct tm local;localtime_r(&epoch,&local);char text[24];strftime(text,sizeof(text),id=="time"?"%H:%M:%S":"%Y-%m-%d",&local);v=text;}else v="--";}else v="--";
