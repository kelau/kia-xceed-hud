...
class Dashboard {
 ...
 private:
  void configure(size_t i,const Widget&w){
    ...
    auto value=values_[i];lv_obj_set_style_text_color(value,lv_color_hex(w.textColor),0);lv_obj_set_style_text_font(value,&lv_font_montserrat_14,0);lv_obj_clear_flag(value,LV_OBJ_FLAG_HIDDEN);
    if(visuals_[i]){lv_obj_del(visuals_[i]);visuals_[i]=nullptr;}
    if(w.visualStyle==WIDGET_BAR){visuals_[i]=lv_bar_create(card);lv_obj_set_size(visuals_[i],max(20,w.w-24),12);lv_obj_align(visuals_[i],LV_ALIGN_BOTTOM_MID,0,-5);lv_bar_set_range(visuals_[i],0,1000);lv_obj_set_style_bg_color(visuals_[i],lv_color_hex(0x1f3b4d),LV_PART_MAIN);lv_obj_set_style_bg_color(visuals_[i],lv_color_hex(0x2dd4bf),LV_PART_INDICATOR);lv_obj_align(value,LV_ALIGN_CENTER,0,-3);}
    else if(w.visualStyle==WIDGET_GAUGE){int size=max(30,min(w.w-20,w.h-28));visuals_[i]=lv_arc_create(card);lv_obj_set_size(visuals_[i],size,size);lv_obj_align(visuals_[i],LV_ALIGN_CENTER,0,8);lv_arc_set_range(visuals_[i],0,1000);lv_arc_set_bg_angles(visuals_[i],135,45);lv_obj_remove_style(visuals_[i],nullptr,LV_PART_KNOB);lv_obj_clear_flag(visuals_[i],LV_OBJ_FLAG_CLICKABLE);lv_obj_set_style_arc_color(visuals_[i],lv_color_hex(0x2dd4bf),LV_PART_INDICATOR);lv_obj_align(value,LV_ALIGN_CENTER,0,8);lv_obj_move_foreground(value);}
    else if(w.visualStyle==WIDGET_LIGHT){visuals_[i]=lv_obj_create(card);lv_obj_set_size(visuals_[i],min(38,max(18,w.h-34)),min(38,max(18,w.h-34)));lv_obj_set_style_radius(visuals_[i],LV_RADIUS_CIRCLE,0);lv_obj_set_style_border_width(visuals_[i],0,0);lv_obj_align(visuals_[i],LV_ALIGN_LEFT_MID,4,8);lv_obj_align(value,LV_ALIGN_CENTER,14,8);}
  ...
  void applyVisual(size_t i,const Widget&w,float raw,bool numeric,bool active){if(!visuals_[i])return;float lo=0,hi=1;bool ranged=numeric&&widgetRange(w.id,lo,hi);int level=ranged?(int)constrain((raw-lo)*1000.0f/(hi-lo),0.0f,1000.0f):(active?1000:0);if(w.visualStyle==WIDGET_BAR)lv_bar_set_value(visuals_[i],level,LV_ANIM_OFF);else if(w.visualStyle==WIDGET_GAUGE)lv_arc_set_value(visuals_[i],level);else if(w.visualStyle==WIDGET_LIGHT){uint32_t color=level>=667?0x22c55e:level>=334?0xf59e0b:0xef4444;lv_obj_set_style_bg_color(visuals_[i],lv_color_hex(color),0);lv_obj_set_style_shadow_color(visuals_[i],lv_color_hex(color),0);lv_obj_set_style_shadow_width(visuals_[i],8,0);}}
