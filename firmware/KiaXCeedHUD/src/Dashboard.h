...
class Dashboard {
 ...
 private:
  void configure(size_t i,const Widget&w){
    ...
    auto title=titles_[i];lv_label_set_text(title,w.id=="text"?"":w.title);lv_obj_set_style_text_color(title,lv_color_hex(w.textColor),0);lv_obj_align(title,LV_ALIGN_TOP_LEFT,0,0);if(w.id=="text"||!w.title[0])lv_obj_add_flag(title,LV_OBJ_FLAG_HIDDEN);else lv_obj_clear_flag(title,LV_OBJ_FLAG_HIDDEN);
    ...
    if(visuals_[i]){lv_obj_del(visuals_[i]);visuals_[i]=nullptr;}if(markers_[i]){lv_obj_del(markers_[i]);markers_[i]=nullptr;}
    ...
    if(w.subdivisions&&(w.visualStyle==WIDGET_BAR||w.visualStyle==WIDGET_GAUGE))createSubdivisions(i,w);
    lv_obj_move_foreground(value);lv_obj_move_foreground(card);
  }
  void createSubdivisions(size_t i,const Widget&w){auto layer=markers_[i]=lv_obj_create(cards_[i]);lv_obj_remove_style_all(layer);lv_obj_set_size(layer,w.w,w.h);lv_obj_set_pos(layer,-12,-12);lv_obj_clear_flag(layer,LV_OBJ_FLAG_CLICKABLE|LV_OBJ_FLAG_SCROLLABLE);for(uint8_t n=1;n<=w.subdivisions;n++){auto mark=lv_obj_create(layer);lv_obj_remove_style_all(mark);lv_obj_set_style_bg_color(mark,lv_color_hex(w.textColor),0);lv_obj_set_style_bg_opa(mark,LV_OPA_70,0);if(w.visualStyle==WIDGET_BAR){int x=12+n*(w.w-24)/(w.subdivisions+1);lv_obj_set_size(mark,1,w.visualThickness+4);lv_obj_set_pos(mark,x,w.h-13-w.visualThickness);}else{int size=max(30,min(w.w-20,w.h-28)),radius=size/2,angle=135+n*270/(w.subdivisions+1);float radians=angle*3.14159265f/180.0f;lv_obj_set_size(mark,3,3);lv_obj_set_style_radius(mark,LV_RADIUS_CIRCLE,0);lv_obj_set_pos(mark,w.w/2+(int)(cosf(radians)*radius)-1,w.h/2+8+(int)(sinf(radians)*radius)-1);}}lv_obj_move_foreground(layer);lv_obj_move_foreground(values_[i]);}
  ...
  lv_obj_t*root_=nullptr;std::array<lv_obj_t*,40>cards_{};std::array<lv_obj_t*,40>titles_{};std::array<lv_obj_t*,40>values_{};std::array<lv_obj_t*,40>visuals_{};std::array<lv_obj_t*,40>markers_{};std::array<std::string,40>ids_{};lv_event_cb_t webAccessCallback_=nullptr;
};}
