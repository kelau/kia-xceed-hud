...
#include <time.h>
#include "Config.h"
extern "C" { extern const lv_font_t hud_font_64,hud_font_80,hud_font_96,hud_font_128,hud_font_160,hud_font_192,hud_font_256,hud_font_320,hud_font_400; }
namespace hud {
class Dashboard {
 public:
  ...
  void rebuild(const Config& c){
    auto oldRoot=root_;cards_.fill(nullptr);titles_.fill(nullptr);values_.fill(nullptr);visuals_.fill(nullptr);for(auto&id:ids_)id.clear();root_=createRoot();populate(root_,c);if(oldRoot)lv_obj_del(oldRoot);
  }
  ...
  void update(const Config& c,const Telemetry&t,uint32_t now){
    for(size_t i=0;i<c.widgets.size();i++){
      ...
      else if(id=="canAge"){uint32_t ageUs=micros()-t.lastCanUs;raw=ageUs/1000.0f;v=ageUs<1000?String(ageUs)+" us":String(raw,1)+" ms";}else if(id=="coordinates"){numeric=false;v=String(t.latitude,5)+", "+String(t.longitude,5);}
 ...
 private:
  ...
  void configure(size_t i,const Widget&w){
    ...
    auto value=values_[i];lv_obj_set_style_text_color(value,lv_color_hex(w.textColor),0);lv_obj_set_style_text_font(value,fontFor(w),0);uint16_t zoom=max(256,(int)w.fontSize*256/(int)fontBaseSize(w));lv_obj_set_style_transform_zoom(value,zoom,0);lv_obj_set_style_transform_pivot_x(value,lv_pct(50),0);lv_obj_set_style_transform_pivot_y(value,lv_pct(50),0);lv_obj_add_flag(value,LV_OBJ_FLAG_OVERFLOW_VISIBLE);lv_obj_clear_flag(value,LV_OBJ_FLAG_HIDDEN);
  ...
  lv_obj_t*root_=nullptr;std::array<lv_obj_t*,40>cards_{};std::array<lv_obj_t*,40>titles_{};std::array<lv_obj_t*,40>values_{};std::array<lv_obj_t*,40>visuals_{};std::array<std::string,40>ids_{};lv_event_cb_t webAccessCallback_=nullptr;
};}
