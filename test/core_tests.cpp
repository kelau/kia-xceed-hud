...
int main(){
  ...
  Telemetry simulated;for(uint8_t i=0;i<5;i++){auto f=simulatedFrame(1000+i*100,i);assert(decodeObd(f,simulated));}assert(simulated.speedKph>0&&simulated.rpm>0&&simulated.coolantC>0&&simulated.engineLoad>0);
