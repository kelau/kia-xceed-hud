...
int main(){
  ...
  Telemetry simulated;int decoded=0,rawFrames=0;for(uint32_t i=0;i<200;i++){auto f=simulatedFrame(1000+i,i);decoded+=decodeObd(f,simulated);rawFrames+=f.id!=0x7E8;}assert(decoded>=70&&rawFrames==100&&simulated.speedKph>0&&simulated.rpm>0&&simulated.coolantC>0&&simulated.engineLoad>0);assert(SIMULATED_CAN_FPS==100&&SIMULATED_CAN_PERIOD_US==10000);
