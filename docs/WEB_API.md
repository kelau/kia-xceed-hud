...
| GET | `/api/frame-detail?key=` | Five-minute frequency, observed ranges, latest bytes and one-second graph samples |
...
| GET | `/api/export.csv` | Stream the rolling five-minute capture as CSV |
| GET | `/api/export.log` | Stream the rolling five-minute capture in SocketCAN/candump format |
