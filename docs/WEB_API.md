...
| GET | `/api/frame-detail?key=` | Two-minute frequency, observed ranges, latest bytes and one-second graph samples |
...
| GET | `/api/export.csv` | Stream the rolling two-minute capture as CSV |
| GET | `/api/export.log` | Stream the rolling two-minute capture in SocketCAN/candump format |
